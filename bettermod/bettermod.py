...
class BetterMod:
    ...
    # BetterMod: Modified by kportalz for use by TT
    ...
    async def error(self, ctx):
        ...
        # it'll check for missing files/permissions

        # bot_member = ctx.message.server.get_member(self.bot.user.id) <- Unused function

        folders = ["data", "data/bettermod/", "data/bettermod/history/", "data/red"]
        files = [
            "data/bettermod/settings.json",
            "data/bettermod/history/{}.json".format(ctx.message.server.id),
            "red/settings.json",
        ]
        ...
        for permission in permissions:
            if {x[0]: x[1] for x in ctx.message.channel.permissions_for(ctx.message.server.me)}[
                permission
            ] is False:
                message_perm += "`{}`\n".format(permission)
                error_perm = 1
    ...
    async def init(self, server, ctx):
        ...
        # this creates default files if they doesn't exist
        ...
        if server.id not in self.settings:
            self.settings[server.id] = {
                ...
                "thumbnail": {
                    ...
                    "warning_embed_softban": "https://i.imgur.com/uhrYzyt.png",
    ...
    async def add_case(self, level, user, reason, timestamp, server, applied, ctx):
        # adds a new case to the settings
        ...
        if level == "Simple":
            ...
            softban_total = history[user.id]["softban-warn"] + 1
            history[user.id]["softban-warn"] = softban_total
        elif level == "Ban":
    ...
    async def check_case(self, msg, i, ctx, user):
        # list all existing warnings, then show them through a reactions menu
        ...
        if not msg:
            if history[user.id]["case{}".format(str(i))]["deleted"] == 1:
                ...
                e.add_field(
                    name="Level",
                    value=history[user.id]["case{}".format(str(i))]["level"],
                    inline=True,
                )
                ...
                e.add_field(
                    name="Date",
                    value=history[user.id]["case{}".format(str(i))]["timestamp"],
                    inline=True,
                )
                e.add_field(
                    name="Reason",
                    value=history[user.id]["case{}".format(str(i))]["reason"],
                    inline=False,
                )
    ...
    async def bmodset(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            l_warn_embed_softban = (
                "None"
                if "warning_embed_softban" in setting["thumbnail"] is False
                else setting["thumbnail"]["warning_embed_softban"]
            )
            l_colour_embed_softban = (
                "None"
                if "warning_embed_softban" in setting["colour"] is False
                else setting["colour"]["warning_embed_softban"]
            )
            l_channel_embed_softban = (
                "None"
                if "softban-warn" in setting["channels"] is False
                else setting["channels"]["softban-warn"]
            )
...
thumbnail's URL pictures:
    ...
    softban warning: {}
    ban warning: {}""".format(
                discord.utils.get(server.roles, id=setting["role"]),
                setting["channels"]["general"],
                setting["channels"]["report"],
                setting["channels"]["simple-warn"],
                setting["channels"]["kick-warn"],
                l_channel_embed_softban,
                setting["channels"]["ban-warn"],
                setting["colour"]["report_embed"],
                l_colour_embed_softban,
                setting["colour"]["warning_embed_kick"],
                setting["colour"]["warning_embed_softban"],
                setting["colour"]["warning_embed_ban"],
                setting["thumbnail"]["report_embed"],
                setting["thumbnail"]["warning_embed_simple"],
                setting["thumbnail"]["warning_embed_kick"],
                l_warn_embed_softban,
    ...
    async def mention(self, ctx, role: str = None):
        ...
        if role is None:
            ...
            await
            self.bot.say("The role mention is now disabled")
            ...
            if object is None:
                await
                self.bot.say("The role cannot be found. Please give exact role name")
                return
            ...
            await
            self.bot.say(
                "The role {} will now be mentioned when a report is sent".format(object.name)
            )
    ...
    async def report_thumbnail(self, ctx, url: str):
        ...
        if not url.endswith((".jpg", ".png", ".gif")) and not url.startswith(
            ...
            await
            self.bot.say("The URL given is not valid")
            return
            ...
            try:
                ...
                await
                self.bot.say(
                    "The new thumbnail for the report embed has been set. If the URL is not valid, no thumbnail will be shown in the embed."
                )
            except:
                await
                self.error(ctx)
    ...
    async def simple_thumbnail(self, ctx, *, url):
        ...
        if not url.endswith((".jpg", ".png", ".gif")) and not url.startswith(
            ...
            try:
                ...
                await
                self.bot.say(
                    "The new thumbnail for the simple warning embed has been set. If the URL is not valid, no thumbnail will be shown in the embed."
                )
            except:
                await
                self.error(ctx)
    ...
    async def kick_thumbnail(self, ctx, url: str):
        ...
        if not url.endswith((".jpg", ".png", ".gif")) and not url.startswith(
            ...
            try:
                ...
                await
                self.bot.say(
                    "The new thumbnail for the kick warning embed has been set. If the URL is not valid, no thumbnail will be shown in the embed."
                )
            except:
                await
                self.error(ctx)
    ...
    async def report_channel(self, ctx, channel: discord.Channel):
        ...
        await
        self.bot.say("Reports will be send to **" + channel.name + "**.")
        try:
            dataIO.save_json("data/bettermod/settings.json", self.settings)
        except:
            await
    ...
    async def simple_warning_channel(self, ctx, channel: discord.Channel):
        ...
        await
        self.bot.say("Simple warnings will be send to **" + channel.name + "**.")
        try:
            dataIO.save_json("data/bettermod/settings.json", self.settings)
        except:
            await
    ...
    async def kick_warning_channel(self, ctx, channel: discord.Channel):
        ...
        await
        self.bot.say("Kick warnings will be send to **" + channel.name + "**.")
        try:
            dataIO.save_json("data/bettermod/settings.json", self.settings)
        except:
            await
    ...
    async def softban_warning_channel(self, ctx, channel: discord.Channel):
        ...
        await
        self.bot.say("Ban warnings will be send to **" + channel.name + "**.")
        try:
            dataIO.save_json("data/bettermod/settings.json", self.settings)
        except:
            await
    ...
    async def report(self, ctx, user: discord.Member = None, *, reason: str = None):
        ...
        if user is None or reason is None:
            try:
                await
                self.bot.send_message(ctx.message.author,
        ...
        if self.settings[server.id][
            "proof"] is True and ctx.message.attachments == [] and "http" not in ctx.message.content:
        ...
        if "http" in ctx.message.content:
            for word in ctx.message.content.split(" "):
                if word.startswith("http"):
                    ...
                    report.add_field(
                        name="Reason",
                        value=reason + "\n\n[Click here to see proof URL]({})".format(word),
                        inline=False,
                    )
                    break
        ...
        if self.settings[server.id]["role"] is None:
            try:
                await
                self.bot.send_message(channel, embed=report)
            except:
                await
                self.error(ctx)
            ...
            try:
                await
                self.bot.send_message(channel, embed=report, content=role.mention)
            except:
                await
                self.error(ctx)
        ...
        if files:
            for file in files:
                try:
                    await
                    self.bot.send_file(channel, file)
                except discord.errors.Forbidden:
                    await
                    self.bot.say(
        ...
        try:
            await
            self.bot.send_message(author, self.settings[server.id]["report_message"].format(
    ...
    async def simple(self, ctx, user: discord.Member, *, reason: str):
        ...
        if not self.settings[server.id]["channels"]["simple-warn"]:
            ...
            channel = discord.utils.get(
                server.channels, id=self.settings[server.id]["channels"]["simple-warn"]
            )
        ...
        try:
            modlog.color = discord.Colour(
                self.settings[server.id]["colour"]["warning_embed_simple"]
            )
        except:
            pass
        ...
        try:
            await
            self.bot.send_message(user, embed=target)
        except:
            modlog.set_footer(
        ...
        await
        self.bot.send_message(channel, embed=modlog)
    ...
    async def kick(self, ctx, user: discord.Member, *, reason: str):
        ...
        self.settings[server.id]["channels"]["kick-warn"]
            ...
            if not self.settings[server.id]["channels"]["kick-warn"]:
                ...
                channel = discord.utils.get(
                    server.channels, id=self.settings[server.id]["channels"]["kick-warn"]
                )
        ...
        try:
            ...
            await
            self.bot.say(
                "I cannot kick this user, they are higher than me in the role hierarchy. Aborting..."
            )
            await
            self.bot.send_message(
    ...
    async def softban(self, ctx, user: discord.Member, *, reason: str):
        ...
        target = discord.Embed(
            description="The moderation team set you a level 3 (softban) warning. "
    ...
    async def ban(self, ctx, user: discord.Member, *, reason: str):
        ...
        self.settings[server.id]["channels"]["ban-warn"]
            ...
            if not self.settings[server.id]["channels"]["ban-warn"]:
                ...
                channel = discord.utils.get(
                    server.channels, id=self.settings[server.id]["channels"]["ban-warn"]
                )
    ...
    async def bcheck(self, ctx, user: discord.Member = None, case: int = 0):
        ...
        if allowed is False:
            await
            self.bot.say("You are not allowed to check others's warnings")
            return

        if user.id not in history:
            if user != author:
                await
                self.bot.say("That user doesn't have any warnings yet")
                return

            else:
                await
                self.bot.say("You don't have any warning yet")
                return
        ...
        if case < 0 or case > history[user.id]["total-warns"]:
            await
            self.bot.say("That case does not exist")
            return
        ...
        if case == 0:
            ...
            i = None
            await
            self.check_case(msg, i, ctx=ctx, user=user)

        else:
            i = case
            await
            self.check_case(msg=None, i=i, ctx=ctx, user=user)
    ...
    async def delete(self, ctx, case: int, user: discord.Member):
        ...
        if user.id not in history:
            await
            self.bot.say("That user does not have any warning yet")
            return
        ...
        if (
            ...
            await
            self.bot.say("That case does not exist or is already deleted")
            return
        ...
        e.add_field(
            name="Reason",
            value=history[user.id]["case{}".format(str(case))]["reason"],
            inline=False,
        )

        if history[user.id]["case{}".format(str(case))]["level"] == "Simple":
            e.add_field(
                name="Total simple warnings",
                value="Before: {}\nAfter: {}".format(
        ...
        if response.reaction.emoji == "✅":
            ...
            try:
                ...
                return
            await
            self.bot.say("The case {} of {} has been deleted".format(str(case), user.name))

        try:
            dataIO.save_json("data/bettermod/settings.json", self.settings)
            dataIO.save_json(
                "data/bettermod/history/{}.json".format(ctx.message.server.id), history
            )
        except:
            await
            self.error(ctx)
    ...
    async def edit(self, ctx, case: int, user: discord.Member, *, reason):
        ...
        await
        self.bot.say("The new reason has been saved")
...
def check_files():
    # ignore_list = {'SERVERS': [], 'CHANNELS': []} <- Never used.
    files = {"settings.json": {}}
...
def check_version_history():
    for file in os.listdir("data/bettermod/history"):
        if file.endswith(".json"):
            ...
            if (
                    "version" not in file_json
            ):  # a log file is not up-to-date (usually means all the files aren't up-to-date)
                file_json["version"] = "1.1"
                # Add here new body
                dataIO.save_json("data/bettermod/history/{}".format(file), file_json)
                print(
                    "Json body of data/bettermod/history/{} was successfully updated".format(file)
                )
